...
------------------------------------------------
✅ Intelligent Query Classification (Keyword-Based Pattern Matching)
✅ Greeting Detection & Handling
❌ Auto Spelling Correction (Disabled - can cause incorrect corrections)
✅ Hybrid Search (Catalogue + FAISS)
✅ General Inquiries (JSON + FAISS)
✅ Web Scraping from Official Library Website
✅ OPAC Fallback
✅ Professional Response Formatting
✅ Comprehensive Error Handling & Logging
✅ Fast & Offline-Ready (No External APIs Required)
...
try:
    ...
    logging.warning("requests library not available - web scraping disabled")
...
_model_cache = {"semantic_search": None, "tfidf_fallback": None}
...
# -------------------- ERROR MESSAGES --------------------
ERROR_PROCESSING_QUERY = "⚠️ Sorry, I encountered an error processing your query. Please try again."
ERROR_SEARCHING_WEBSITE = "⚠️ Sorry, I encountered an error searching the website. Please try again."
ERROR_SEARCHING_BOOKS = "⚠️ Sorry, I encountered an error searching for books. Please try again or contact the library helpdesk for assistance."
ERROR_UNEXPECTED = "⚠️ Sorry, I encountered an unexpected error. Please try again or contact support if the problem persists."
...
def audit_log_admin_activity(activity: str, details: dict, admin_user: str = "admin", client_ip: str = "default"):
    """
    Log admin panel activities for security audit and monitoring.
    Captures all admin actions like FAISS rebuilds, file uploads, cache clears, etc.
    
    Args:
        activity: Type of admin activity (e.g., "faiss_rebuild", "file_upload", "cache_clear")
        details: Dictionary with activity-specific details
        admin_user: Admin username performing the action
    ...
    try:
        log_entry = {
            ...
            "client_ip": client_ip[:10] + "***" if client_ip != "default" else "default",
            "details": details
        }
        ...
        logger.info(f"Admin activity logged: {activity} by {admin_user}")
            
    except Exception as e:
        # Don't fail admin action if logging fails
        logger.error(f"Admin activity logging failed: {e}")
...
# health_check() function removed - was unused
...
# ========================
# sanitize_input() function removed - was unused
...
def markdown_to_html(text: str) -> str:
    """
    Convert markdown formatting to HTML for proper display in widget.
    ...
    text = re.sub(r'\[(.+?)\]\((.+?)\)', r'<a href="\2" target="_blank" style="color: #10b981; text-decoration: underline;">\1</a>', text)
...
def auto_correct_spelling(query):
    ...
    try:
        ...
        preserve_words = ['timings', 'ebooks', 'eresources', 'opac', 'scopus',
                          ...
                          'iit', 'ropar', 'isbn', 'doi', 'issn', 'dspace',
                          'website', 'shodhganga', 'pressreader', 'website']
...
def check_book_availability_opac(title="", author="", isbn="", accession_numbers=None, cache_timeout=300, force_refresh=False):
    ...
    # Create cache key from search parameters - include accession numbers
    cache_key_parts = [title, author, isbn]
    if accession_numbers:
        cache_key_parts.extend(accession_numbers)
    cache_key = "|".join(cache_key_parts).strip("|")
    if not cache_key:
        return None
        
    cache_file = Path("cache/opac_cache.json")
    cache_file.parent.mkdir(exist_ok=True)
    ...
    try:
        ...
        # Use accession numbers if available (most accurate)
        if accession_numbers and len(accession_numbers) > 0:
            # Search using "bc:" prefix for accession numbers (only add if not already present)
            accession_queries = []
            for acc in accession_numbers:
                acc = acc.strip()
                if acc.startswith('bc:'):
                    accession_queries.append(acc)  # Don't add quotes if already bc:
                else:
                    accession_queries.append(f'bc:"{acc}"')
            if accession_queries:
                search_terms.extend(accession_queries)
        
        # Fallback to other search methods if no accession numbers
        if not search_terms:
            if title:
        ...
        # Use OR logic to search for any of the accession numbers or fallback terms
        ...
        logger.info(f"🔍 Searching OPAC for: {search_query}")
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
        }
        ...
        # Check if this is a book details page (redirected from bc: search)
        if 'opac-detail.pl' in response.url:
            # Parse book details page
            availability_info["total_results"] = 1  # One book found
            
            # Look for item table rows
            table_rows = soup.find_all('tr')
            for row in table_rows:
                cells = row.find_all(['td', 'th'])
                if len(cells) >= 6:  # Table has 7 columns, we need at least status column
                    row_text = [cell.get_text(strip=True) for cell in cells]
                    
                    # Status is typically in column 3 (0-indexed: 0=type, 1=library, 2=call#, 3=status, 4=date_due, 5=barcode)
                    status_cell = row_text[3] if len(row_text) > 3 else ""
                    
                    # Check if this is a data row (not header) and has status info
                    if status_cell and status_cell in ['Available', 'Checked out', 'In transit', 'On hold']:
                        availability_info["total_copies"] += 1
                        
                        if status_cell == 'Available':
                            availability_info["available_copies"] += 1
                        
                        # Add book info
                        book_info = {
                            "title": title or "Unknown Title",
                            "item_type": row_text[0] if len(row_text) > 0 else "Unknown",
                            "collection": row_text[1] if len(row_text) > 1 else "Unknown", 
                            "status": status_cell,
                            "barcode": row_text[5] if len(row_text) > 5 else "",
                            "call_number": row_text[2].strip() if len(row_text) > 2 else "",
                            "due_date": row_text[4] if len(row_text) > 4 else ""
                        }
                        availability_info["details"].append(book_info)
        else:
            # Parse search results page (original logic)
            # Look for result count
            result_text = soup.get_text()
            match = re.search(r'(\d+)\s+results?\s+found', result_text, re.IGNORECASE)
            if match:
                availability_info["total_results"] = int(match.group(1))
            
            # Look for individual book records
            book_records = soup.find_all('div', class_=re.compile(r'result', re.I))
            
            for record in book_records[:10]:  # Check first 10 results
                book_info = {}
                
                # Extract title - look for links or headings
                title_elem = record.find(['a', 'h3', 'h4'], href=re.compile(r'biblio|detail', re.I))
                if not title_elem:
                    title_elem = record.find(['a', 'h3', 'h4'])
                if title_elem:
                ...
                if available_count > 0:
                    ...
                    book_info["status"] = "Issued"
                elif 'reference' in record_text.lower():
                    book_info["status"] = "Reference Only"
                else:
                    book_info["status"] = "Unknown"
                
                if book_info:
                    availability_info["details"].append(book_info)
        
        # Determine overall status
        if availability_info["available_copies"] > 0:
            availability_info["status"] = "available"
        elif availability_info["total_copies"] > 0:
            availability_info["status"] = "issued"
        else:
        ...
        logger.error(f"❌ OPAC check failed: {e}")
        return None

@lru_cache(maxsize=500)
def classify_query(query):
    """
    Classify user query into categories: 'book', 'general', or 'greeting'.
    Uses keyword-based pattern matching for fast, accurate classification.
    
    Returns:
        str: 'book', 'general', or 'greeting'
    """
    ...
    greeting_patterns = [
        ...
        r'\b(how are you|how r u|how do you do|whats up|what\'s up|wassup)\b',
        r'\b(how is it going|hows it going)\b',
        # Casual
        r'\b(yo|sup|hiya|howdy)\b',
        # Polite
        r'\b(namaste|namaskar)\b',
    ]
    
    # Check if it's just a greeting (short query with greeting words)
    if len(query_lower.split()) <= 6:  # Short queries likely to be greetings
        for pattern in greeting_patterns:
            if re.search(pattern, query_lower):
                logger.info(f"👋 Fallback: GREETING detected")
                return 'greeting'
    ...
    # If query is very short (1-2 words) and contains no general keywords, likely a general search
    words = query_lower.split()
    if len(words) <= 3 and not any(kw in query_lower for kw in general_keywords):
        logger.info(f"📋 Fallback classified '{query}' as GENERAL QUERY (short query, likely general question)")
        return 'general'
    
    # Default: treat as general search (most common use case)
    logger.info(f"📋 Fallback classified '{query}' as GENERAL QUERY (default)")
    return 'general'
...
def get_general_answer(query):
    ...
    1. ✅ Exact match in JSON (for known queries)
    2. 🔍 Fuzzy string matching (for typos)
    3. 🚀 FAISS semantic search (BEST - understands intent)
    ...
    try:
        ...
        # STRATEGY 1: Exact match on original and alternatives
...
def _load_faiss_resources_generic(cache_dict, loading_flag_ref, index_file, mapping_file, log_message, timeout_message):
    """
    Generic FAISS resource loader to eliminate code duplication.
    ...
    Args:
        cache_dict: Cache dictionary to store loaded resources
        loading_flag_ref: Reference to loading flag variable
        index_file: Path to FAISS index file
        mapping_file: Path to mapping pickle file
        log_message: Loading log message
        timeout_message: Timeout error message

    Returns:
        tuple: (model, index, mapping)
    """
    # Get the global loading flag
    loading_flag = loading_flag_ref

    # Return cached if available
    if 'model' in cache_dict:
        return cache_dict['model'], cache_dict['index'], cache_dict['mapping']
    ...
    try:
        ...
        cache_dict['model'] = model
        cache_dict['index'] = index
        cache_dict['mapping'] = mapping

        elapsed = time.time() - start
        logger.info(f"✅ FAISS resources loaded and cached in {elapsed:.2f}s")

        return model, index, mapping
    finally:
        loading_flag_ref = False

def _load_faiss_resources():
    """Load and cache FAISS model, index, and mapping for catalogue search."""
    global _faiss_loading
    return _load_faiss_resources_generic(
        _faiss_cache, _faiss_loading, INDEX_FILE, MAPPING_FILE,
        "🔄 Loading FAISS catalogue index (first query only)...",
        "FAISS loading timeout"
    )

# Pre-load FAISS resources on import to reduce first-search latency
try:
    import threading
    def preload_faiss():
        try:
            logger.info("🔄 Pre-loading FAISS resources in background...")
            start_time = time.time()
            _load_faiss_resources()
            load_time = time.time() - start_time
            logger.info(f"✅ FAISS resources pre-loaded in {load_time:.2f}s")
        except Exception as e:
            logger.warning(f"Failed to pre-load FAISS: {e}")
    
    preload_thread = threading.Thread(target=preload_faiss, daemon=True)
    preload_thread.start()
except Exception as e:
    logger.warning(f"Could not start FAISS preloading: {e}")
...
def _load_general_faiss_resources():
    ...
    return _load_faiss_resources_generic(
        _general_faiss_cache, _general_faiss_loading, GENERAL_QUERIES_INDEX_FILE, GENERAL_QUERIES_MAPPING_FILE,
        "🔄 Loading general queries FAISS index...",
        "General FAISS loading timeout"
    )
    ...
    if all_results:
        ...
        for book in merged:
            if NANDU_WEBSCRAPE:  # Only check if web scraping is enabled
                ...
                # Extract accession numbers - they might be a string or list
                accession_numbers = book.get('accessions', book.get('accession_numbers', []))
                if isinstance(accession_numbers, str):
                    # Split comma-separated string into list
                    accession_numbers = [acc.strip() for acc in accession_numbers.split(',') if acc.strip()]
                
                availability = check_book_availability_opac(
                    title=title, 
                    author=author, 
                    isbn=isbn,
                    accession_numbers=accession_numbers,
                    force_refresh=realtime_availability
                )
                
                if availability:
                    book['opac_availability'] = availability
...
# Main Query Handler 
# -------------------------
def generate_dynamic_book_intro(query: str, book_intent: str, count: int) -> str:
    """
    Generate varied, conversational introductions for book search results.
    Uses different templates to make responses feel more natural.
    """
    import random

    if book_intent == "author":
        author_intros = [
            f"📚 **Excellent! I found {count} book{'s' if count != 1 else ''} by {query} in our collection:**",
            f"📚 **Great choice! Here are {count} book{'s' if count != 1 else ''} written by {query}:**",
            f"📚 **Perfect! I located {count} title{'s' if count != 1 else ''} from {query} in our library:**"
        ]
        return random.choice(author_intros)

    elif count == 1:
        single_intros = [
            f"📚 **Perfect! I found exactly what you're looking for:**",
            f"📚 **Great news! Here's the book you were searching for:**",
            f"📚 **Found it! This matches your search perfectly:**"
        ]
        return random.choice(single_intros)

    elif count <= 3:
        few_intros = [
            f"📚 **I found {count} books matching '{query}':**",
            f"📚 **Here are {count} relevant books for '{query}':**",
            f"📚 **I located {count} titles related to your search:**"
        ]
        return random.choice(few_intros)

    else:
        many_intros = [
            f"📚 **I found {count} books related to '{query}'. Here are the most relevant ones:**",
            f"📚 **Great selection! Here are {count} books matching your search (showing top results):**",
            f"📚 **I discovered {count} titles for '{query}'. Here are the best matches:**"
        ]
        return random.choice(many_intros)


def format_books_for_widget(results):
    """
    Format book results as JSON for widget display with availability in separate column.
    Returns a list of book objects with standardized fields.
    """
    formatted_books = []
    ...
    for book in results:
        ...
        book_data = {
            ...
            'isbn': book.get('ISBN', book.get('isbn', '')),
            'year': book.get('Year', book.get('copyrightdate', '')),
            'publisher': book.get('Publisher', book.get('publishercode', '')),
            'copies': book.get('copies', 1),
            'call_numbers': book.get('call_numbers', []),
            'accessions': book.get('accessions', []),
            'summary': book.get('summary', '')
        }
        
        # Add availability information
        opac_data = book.get('opac_availability')
        if opac_data:
            book_data['availability'] = {
                'status': opac_data.get('status', 'unknown'),
                'available_copies': opac_data.get('available_copies', 0),
                'total_copies': opac_data.get('total_copies', 0)
            }
            
            # Add item details from OPAC
            item_details = []
            for detail in opac_data.get('details', []):
                item_info = {
                    'item_type': detail.get('item_type', 'Unknown'),
            ...
            book_data['item_details'] = item_details
        else:
            book_data['availability'] = {
                'status': 'unknown',
                'available_copies': 0,
...
def generate_clarification_response(query: str, context: str = "general") -> str:
    """
    Generate context-aware clarification responses when the bot is unsure.
    Provides specific follow-up questions based on query content and context.
    """
    query_lower = query.lower().strip()

    # Analyze query for context clues
    has_time_words = any(word in query_lower for word in ['time', 'timing', 'hours', 'open', 'close', 'schedule', 'when'])
    has_book_words = any(word in query_lower for word in ['book', 'books', 'author', 'title', 'isbn', 'find', 'search'])
    has_service_words = any(word in query_lower for word in ['borrow', 'return', 'fine', 'penalty', 'issue', 'renew', 'membership'])
...
def get_nandu_response(q, search_mode: str = "auto", client_ip: str = "default"):
    ...
    # Step 1: Clean query before auto-correction (remove extra punctuation)
    import re
    cleaned_query = re.sub(r'[^\w\s]', '', original_query).strip()
    
    # Step 2: Auto-correct spelling DISABLED (can cause incorrect corrections of technical terms)
    # corrected_query = auto_correct_spelling(cleaned_query)
    corrected_query = cleaned_query  # Skip auto-correction
    ...
    try:
        ...
        if accession_match:
            accession_number = f"bc:{accession_match.group(1)}"
            logger.info(f"🔍 Detected accession number query: {accession_number}")
            
            # Call OPAC directly for this accession number
            availability = check_book_availability_opac(accession_numbers=[accession_number], force_refresh=True)
            
            if availability and availability.get('details'):
                ...
                response = f"📚 **Book Details for Accession {accession_number}:**\n\n"
                response += f"**Status:** {available_copies}/{total_copies} copies available\n\n"
                
                for i, item in enumerate(details, 1):
                    status_color = "🟢" if item['status'] == 'Available' else "🔴"
                    response += f"**Copy {i}:** {status_color} {item['status']}\n"
                    response += f"• **Type:** {item['item_type']}\n"
                    if item['collection'] != 'Unknown':
                        response += f"• **Collection:** {item['collection']}\n"
                    response += f"• **Barcode:** {item['barcode']}\n"
                    response += f"• **Call Number:** {item['call_number']}\n\n"
                
                # Add JSON for widget
                widget_data = [{
                    'title': f"Book with Accession {accession_number}",
                ...
                import json
                json_block = f"\n\n```json\n{json.dumps(widget_data, ensure_ascii=False)}\n```"
                
                return response + json_block
            else:
                return f"⚠️ I couldn't find details for accession number {accession_number}. Please check the number and try again."
        
        # Check for pure numeric queries (potential accession numbers)
        pure_numeric_match = re.search(r'^\s*(\d+)\s*$', original_query.strip())
        if pure_numeric_match and len(pure_numeric_match.group(1)) >= 4:  # Accession numbers are typically 4+ digits
            accession_number = f"bc:{pure_numeric_match.group(1)}"
            logger.info(f"🔍 Detected potential accession number query: {accession_number}")
            ...
            if availability and availability.get('details'):
                ...
                response = f"📚 **Book Details for Accession {pure_numeric_match.group(1)}:**\n\n"
                response += f"**Status:** {available_copies}/{total_copies} copies available\n\n"
                ...
                for i, item in enumerate(details, 1):
                    ...
                    if item.get('due_date'):
                        response += f"• **Due Date:** {item['due_date']}\n"
                    response += "\n"
                
                # Add JSON for widget
                widget_data = [{
                    'title': f"Book with Accession {pure_numeric_match.group(1)}",
                    'item_details': details,
                    'availability': {
                        'status': availability.get('status', 'unknown'),
                        'available_copies': available_copies,
                ...
                # If not found as accession, fall through to normal book search
                logger.info(f"⚠️ Accession {accession_number} not found, falling through to normal search")
        ...
        # Step 5: Handle based on classification or forced mode
        if query_type == 'library':
            ...
            try:
                ...
                if general:
                    ...
                    prefix = _get_response_prefix(query.lower())
                    
                    # Convert markdown to HTML
                    formatted_answer = markdown_to_html(answer)
                    return prefix + formatted_answer
                else:
                    logger.info("⚠️ No match in library queries JSON")
                    return "🤔 <strong>I couldn't find specific information about that.</strong><br><br><strong>Here are some options:</strong><br>• Try rephrasing your question<br>• Visit the <a href='https://www.iitrpr.ac.in/library/' target='_blank' style='color: #10b981; text-decoration: underline;'>official library website</a><br>• Email the library staff at <strong>libraryhelpdesk@iitrpr.ac.in</strong><br>• Ask me about library hours, borrowing rules, or how to search for books!"
                    
            except Exception as e:
                logger.error(f"Error handling library query: {e}")
                return ERROR_PROCESSING_QUERY
            ...
            try:
                ...
                if website_data:
                    ...
                    if website_result:
                        ...
                        formatted_answer = markdown_to_html(website_result['answer'])
                        return f"💡 {formatted_answer}"
                
                logger.info("⚠️ No match on website")
                return "🤔 <strong>I couldn't find this information on the website.</strong><br><br>Please:<br>• Try different keywords<br>• Visit the <a href='https://www.iitrpr.ac.in/library/' target='_blank' style='color: #10b981; text-decoration: underline;'>official library website</a> directly<br>• Contact the library staff at <strong>library@iitrpr.ac.in</strong>"
                    
            except Exception as e:
                logger.error(f"Error handling website search: {e}")
                return ERROR_SEARCHING_WEBSITE
            ...
            try:
                ...
                if website_data:
                    ...
                    if website_result:
                        logger.info("✅ Found answer from website")
                        formatted_answer = markdown_to_html(website_result['answer'])
                        return f"🌐 <strong>From the Library Website:</strong><br><br>{formatted_answer}"
            ...
            try:
                ...
                results = hybrid_book_search(clean_q, intent=book_intent, realtime_availability=True)
                ...
                if results:
                    ...
                    # Add JSON block for widget to parse
                    import json
                    json_block = f"\n\n```json\n{json.dumps(widget_books, ensure_ascii=False)}\n```"
                    
                    return response_text + json_block
                else:
                    # No results found - provide helpful suggestions
                    logger.info("⚠️ No book results found")
                    
                    # Use dynamic clarification for book search failures
                    return generate_clarification_response(clean_q, "book")
                    
            except Exception as e:
                logger.error(f"Error handling book query: {e}")
                track_error(e, "get_nandu_response:book_query")
                response = ERROR_SEARCHING_BOOKS
                success = False
                return response
...
# ========================
# Lazy-load heavy models on first use to speed up startup and avoid
# unnecessary GPU/CPU overhead for simple general queries.
logger.info("🚀 Nandu Brain module loaded (lazy model init)")